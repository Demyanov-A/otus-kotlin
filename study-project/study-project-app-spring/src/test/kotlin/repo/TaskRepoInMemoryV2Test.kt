...
@WebFluxTest(
    TaskControllerV2Fine::class, TaskConfig::class,
    properties = ["spring.main.allow-bean-definition-overriding=true"]
)
@Import(RepoInMemoryConfig::class)
internal class TaskRepoInMemoryV2Test : TaskRepoBaseV2Test() {
    @Autowired
    ...
    fun tearUp() {
        ...
        coEvery { testTestRepo.createTask(capture(slotAd)) } coAnswers { repo.createTask(slotAd.captured) }
        coEvery { testTestRepo.readTask(capture(slotId)) } coAnswers { repo.readTask(slotId.captured) }
        coEvery { testTestRepo.updateTask(capture(slotAd)) } coAnswers { repo.updateTask(slotAd.captured) }
